...
def fetch_medal_tally(df, year, country):
    ...
    flag = 0
    if year == 'Overall' and country == 'All the Countries':
        temp_df = medal_df
    if year == 'Overall' and country != 'All the Countries':
        flag = 1
...
def country_year_list(df):
    ...
    country.sort()
    country.insert(0, 'All the Countries')
...
def number_of_events_over_time(df):
    ...
    events_over_time.rename(columns={'Year': 'Edition', 'count': 'Event'}, inplace=True)
    return events_over_time

def number_of_athletes_over_time(df):
